...
from preprocessors import BinaryFourLabel
...
TRAIL_ID = 'DE-3DGAN_finetune_Classifier_four' + str(CFG.FOLD)
...
label_preprocessors = {'label': Sequence([BinaryFourLabel()])}
...
preprocessors_results = DEAPDataset(
    ...
    feature_preprocessors)('./dataset/deap_binary_four_dataset.pkl')
...
class FrameDataset(Dataset):
    def __init__(self, trial_ds, device='cuda'):
        ...
        gan_model_state_dict = torch.load(
            './parameters/DE-3DGAN_3D_UNet_GAN_four.pth')
        ...
        with torch.no_grad():
            for idx in range(len(trial_ds)):
                ...
                de = (de * de_sigma + de_mu).unsqueeze(0).cuda()
                gen = self.G(de)                 # -> (1,60,128,9,9)
                gen = gen.squeeze(0).cpu()       # (60,128,9,9)
...
def train_test_split(dataset,
    ...
    if not os.path.exists(kfold_split_index_path):
        ...
        for i, (train_index, test_index) in enumerate(
                kfold.split(indices, dataset.lbl)):
            index_dict[i] = {
...
class Trainer():
    ...
    def validation_epoch_end(self, outputs):
        ...
        y     = torch.cat(y,     dim=0)
        acc = self._accuracy(y_hat.cuda(), y.cuda())
        logger.info(f"[VAL] Average ACC at epoch end is {acc:.4f}")
        return {'val_acc': acc}
    ...
    def fit(self):
        for i in tqdm(range(self._trainer_kwargs['max_epochs'])):
            self._train(i)
            self._validate(i)
        logger.info(f"[VAL] Final ACC: {self._validate()['val_acc']:.4f}")
    ...
    def load(self):
        ...
        if os.path.exists('./parameters/DE-3DGAN_SA-ST_Net_Classifier_four' +
                          str(CFG.FOLD) + '.pth'):
            c_model_state_dict = torch.load(
                './parameters/DE-3DGAN_SA-ST_Net_Classifier_four' + str(CFG.FOLD) +
                '.pth')
...
if __name__=='__main__':
    ...
    ds = FrameDataset(ds)
    G  = Generator3D_UNet()
    C  = Classifier(num_classes=4, in_channels=128)
