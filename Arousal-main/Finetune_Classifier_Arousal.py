...
from preprocessors import BinaryArousalLabel
...
TRAIL_ID = 'DE-3DGAN_finetune_Classifier_Arousal' + str(CFG.FOLD)
...
label_preprocessors = {'label': Sequence([BinaryArousalLabel()])}
...
class FrameDataset(Dataset):
    def __init__(self, trial_ds, device='cuda'):
        ...
        gan_model_state_dict = torch.load(
            './parameters/DE-3DGAN_3D_UNet_GAN_arousal.pth')
...
class Trainer():
    ...
    def load(self):
        ...
        if os.path.exists('./parameters/DE-3DGAN_SA-ST_Net_Classifier_arousal' + str(CFG.FOLD) + '.pth'):
            c_model_state_dict = torch.load(
                './parameters/DE-3DGAN_SA-ST_Net_Classifier_arousal' + str(CFG.FOLD) +
                '.pth')
